...
SAMPLE_SCREEN_X = 20
SAMPLE_SCREEN_Y = 10
font = os.path.join(b'gui', b'data', b'fonts', b'consolas10x10_gs_tc.png')
...
if True:
    ...
    libtcod.parser_run(parser, os.path.join(b'gui', b'data', b'cfg', b'sample.cfg'))
    print(('bool_field : ', \
    ...
    if path_using_astar:
        ...
        for y in range(SAMPLE_SCREEN_HEIGHT):
            for x in range(SAMPLE_SCREEN_WIDTH):
                if smap[y][x] != '#':
                    libtcod.console_set_char_background(sample_console, x, y,
                                                        libtcod.color_lerp(fov_light_ground, fov_dark_ground,
                                                                           0.9 * libtcod.dijkstra_get_distance(
                                                                               path_dijk, x, y) / path_dijk_dist),
                                                        libtcod.BKGND_SET)
...
def render_name(first, key, mouse):
    ...
    if ng_nbsets == 0:
        ...
        for file in os.listdir(b'gui/data/namegen'):
            if file.find(b'.cfg') > 0:
                libtcod.namegen_parse(os.path.join(b'gui', b'data', b'namegen', file))
        # get the sets list
...
if numpy_available:
    (xc, yc) = np.meshgrid(list(range(SCREEN_W)), list(range(SCREEN_H)))
...
while not libtcod.console_is_window_closed():
    ...
    for i in range(libtcod.NB_RENDERERS):
        if i == cur_renderer:
            ...
            libtcod.console_set_default_foreground(None, libtcod.grey)
            libtcod.console_set_default_background(None, libtcod.black)
        libtcod.console_print_ex(None, 42, 46 - (libtcod.NB_RENDERERS - i), libtcod.BKGND_SET, libtcod.LEFT,
                                 renderer_name[i])
